...
class Myfone20csv_Data():
    ...
    def Myfone_20_Data(self, supplier, GroupID, path, UserID):
        ...
        try:
            ...
            logger.info('===Myfone20_Data SUCCESS===')
            return 'success'
    ...
    def parserData(self,table,row_index,GroupID,UserID,supplier):
        try:
            ...
            self.sale.setC_Product_id(row[7])
            self.sale.setProduct_name(row[8].decode('big5').encode('utf-8'))
            self.sale.setQuantity(row[11])
            ...
            self.customer.setPhone(row[15].lstrip('=').strip('"'))
            self.customer.setMobile(row[16].lstrip('=').strip('"'))
...
if __name__ == '__main__':
    myfone = Myfone20_Data()
    groupid = ""
    groupid='cbcc3138-5603-11e6-a532-000d3a800878'
    print myfone.Myfone_20_Data('myfone',groupid, u'C:\\Users\\10509002\\Documents\\電商檔案\\網購平台訂單資訊\\Myfone\\2015.01.05\\deliveryA.csv','system')
