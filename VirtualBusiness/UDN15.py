...
class Udn15_Data():
    ...
    # 預期要找出欄位的索引位置的欄位名稱
    # 悠活原力
    TitleTuple = (u'訂單編號', u'訂購日期', u'商品編號', u'商品名稱+規格尺寸', u'訂購數量',
                  u'售價(促銷價)')
    ...
    def Udn_15_Data(self, supplier, GroupID, path, UserID):

        try:

            logger.debug("===Udn15_Data===")
            ...
            logger.debug('===Udn15_Data finally===')
    ...
    def parserData(self,table,row_index,GroupID,UserID,supplier):
        try:
            ...
            self.sale.setTrans_list_date_YYYYMMDD(table.cell(row_index, self.TitleList.index(self.TitleTuple[1])).value)
            self.sale.setSale_date_YYYYMMDD(table.cell(row_index, self.TitleList.index(self.TitleTuple[1])).value)
            self.sale.setC_Product_id(str(table.cell(row_index, self.TitleList.index(self.TitleTuple[2])).value).split('.')[0])
            self.sale.setProduct_name(table.cell(row_index, self.TitleList.index(self.TitleTuple[3])).value)
...
if __name__ == '__main__':
    ...
    print udn.Udn_15_Data('udn',groupid,u'C:\\Users\\10509002\\Documents\\電商檔案\\網購平台訂單資訊\\UDN\\2015.01.12\\Order_20150112111347401.xls','system')
