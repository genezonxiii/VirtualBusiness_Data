...
class Momo24_Data1():
    ...
    # 預期要找出欄位的索引位置的欄位名稱
    TitleTuple = (u'訂單編號', u'收件人姓名', u'印單日', u'原廠編號',u'品名',
                  u'數量', u'貨運公司\n出貨地址', u'預計出貨日')
    TitleList = []
    ...
    def Momo_24_Data1(self, supplier, GroupID, path, UserID):

        try:

            logger.debug("===Momo24_Data1===")
            ...
            logger.debug('===Momo24_Data1 finally===')
    ...
    def parserData(self,table,row_index,GroupID,UserID,supplier):
        try:
            ...
            self.sale.setTrans_list_date_YYYYMMDD(table.cell(row_index, self.TitleList.index(self.TitleTuple[2])).value)
            self.sale.setSale_date_YYYYMMDD(table.cell(row_index, self.TitleList.index(self.TitleTuple[2])).value)
            self.sale.setC_Product_id(str(table.cell(row_index, self.TitleList.index(self.TitleTuple[3])).value).split('.')[0])
            self.sale.setProduct_name(table.cell(row_index, self.TitleList.index(self.TitleTuple[4])).value)
            self.sale.setQuantity(table.cell(row_index, self.TitleList.index(self.TitleTuple[5])).value)
            self.sale.setPrice(-0.1)
            ...
            self.customer.setAddress(table.cell(row_index, self.TitleList.index(self.TitleTuple[6])).value)
...
if __name__ == '__main__':
    ...
    print momo.Momo_24_Data1('momo',groupid,u'C:\\Users\\10509002\\Documents\\電商檔案\\網購平台訂單資訊\\momo\\2016.09.10\\出貨中（已印單）訂單查詢-第三方物流.xls','system')
