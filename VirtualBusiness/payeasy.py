    ...
    def payeasy_Data(self,supplier,GroupID,path,UserID):
        ...
        for row_index in range(1,table.nrows):
            for col_index in range(0,num_cols):
                aes=aes_data()
                OrderNo = table.cell(row_index, 6).value[0:13]
                ...
                TurnDate = datetime.datetime.strptime(str(table.cell_value(row_index, 5)),'%Y/%m/%d %H:%M')
                ...
                Tel = table.cell(row_index, 18).value
                ...
                Add = table.cell(row_index, 16).value + table.cell(row_index, 17).value
                ClientAdd = aes.AESencrypt("p@ssw0rd", Add.encode('utf8'), True)
                PartNo = str(table.cell(row_index, 9).value).split('.')[-1]
                PartName = table.cell(row_index, 8).value
                PartQuility = table.cell(row_index, 11).value
                PartPrice = table.cell(row_index, 12).value
            ...
            if orderexist != []:
                ...
                updateSaleValue = (UserID,PartName,PartNo,PartQuility,PartPrice,None,None,TurnDate,None,None, ShipmentDate , orderexist[0][0] )
                ...
                SaleSQL = (GroupID, OrderNo, UserID, PartName, PartNo, orderexist[0][0],
                            orderexist[0][1], PartQuility, PartPrice, None, None, TurnDate, None,
        ...
        logging.info('===payeasy_Data SUCCESS===')
        return 'success'
