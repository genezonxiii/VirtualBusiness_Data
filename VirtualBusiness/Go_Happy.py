...
class GoHappy_Data():
    ...
    def GoHappy_Data(self,supplier,GroupID,path,UserID):
        ...
        with open(path, 'rb') as f:
            ...
            for row in reader:
                if ignore:
                    aes=aes_data()
                    OrderNo = row[6]
                    Name = str(row[18].split("'")[-1]).decode('big5')
                    ClientName = aes.AESencrypt("p@ssw0rd", Name.encode('utf-8'), True)
                    Tel = str(row[21].split("'")[-1]).decode('big5')
                    ClientTel = aes.AESencrypt("p@ssw0rd", Tel, True)
                    Phone = str(row[22].split("'")[-1]).decode('big5')
                    ClientPhone = aes.AESencrypt("p@ssw0rd", Phone, True)
                    Add = row[20]
                    ClientAdd = aes.AESencrypt("p@ssw0rd", Add, True)
                    PartName = (row[9].split("'")[-1]).decode('big5')
                    PartNo = row[11].split("'")[-1]
                    PartTotalPrice=int(str(row[16].split('.')[-1]).split("'")[0])
                    PartQuility=row[15].split("'")[-1]
                    strTurnDate = row[8]
        ...
        logging.info('===GoHappy_Data SUCCESS===')
        return 'success'
