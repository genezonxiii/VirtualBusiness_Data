...
class Myfone19_Data():
    ...
    def parserData(self,table,row_index,GroupID,UserID,supplier):
        try:
            ...
            self.sale.setTrans_list_date(row[0])
            self.sale.setSale_date(row[0])
            ...
            self.sale.setName(row[13].decode('big5').encode('utf-8'))

            self.customer.setGroup_id(GroupID)
            self.customer.setName(row[13].decode('big5').encode('utf-8'))
            self.customer.setPhone(row[14])
            self.customer.setMobile(row[15])
            self.customer.setPost(None)
            self.customer.setAddress(row[16].decode('big5').encode('utf-8'))
        except Exception as e :
            print e.message
...
if __name__ == '__main__':
    ...
    print myfone.Myfone_19_Data('myfone',groupid, u'C:\\Users\\10509002\\Documents\\電商檔案\\網購平台訂單資訊\\Myfone\\2014\\2014.11.03\\deliveryA.csv','system')
