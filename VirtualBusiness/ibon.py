    ...
    def IBON_Data(self,supplier,GroupID,path,UserID):
        ...
        with open(path, 'rb') as f:
            ...
            for row in reader:
                if ignore:
                    aes=aes_data()
                    OrderNo = row[8].split("'")[-1]
                    Name = str(row[13].split("'")[-1]).decode('big5')
                    ClientName = aes.AESencrypt("p@ssw0rd", Name.encode('utf-8'), True)
                    Tel = str(row[12].split("'")[-1]).decode('big5')
                    ClientTel = aes.AESencrypt("p@ssw0rd", Tel, True)
                    PartName = (row[22].split("'")[-1]).decode('big5')
                    PartNo = row[21].split("'")[-1]
                    PartTotalPrice=int(str(row[26].split('.')[-1]).split("'")[0])
                    PartQuility=row[25].split("'")[-1]
                    ...
                    CustomereSQL = (str(uuid.uuid4()), GroupID, ClientName, None, ClientTel, None,None,None,None,None)
                    ...
                    if result!=[]:
                        ...
                        if Name.encode('utf-8') in Name_compare :
                            print "update"
                            mysqlconnect.cursor.execute(CustomereSQLupd,(None, ClientTel, None,None,None,None,None,GroupID,x[1]))
                        else:
                            print "select insert"
        ...
        logging.info('===IBON_Data SUCCESS===')
        return 'success'
