    ...
    def Yahoo_DataS3(self,supplier,GroupID,path,UserID):
        ...
        for row_index in range(1,table.nrows):
            for col_index in range(0,num_cols):
                ...
                TurnDate = datetime.datetime.strptime(str(table.cell_value(row_index, 12)),'%Y/%m/%d %H:%M')
                ShipmentDate = datetime.datetime.strptime(str(table.cell_value(row_index, 16)),'%Y/%m/%d %H:%M')
        ...
        logging.info('===Yahoo_DataS3 SUCCESS===')
        return 'success'
    # mongoDB storage   ?�穿????��?????�賣?��?????��???mongoOrder or mongoClient
