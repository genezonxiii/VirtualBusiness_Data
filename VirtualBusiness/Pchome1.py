...
class PCHome_Data():
    ...
    def PCHome_Data(self,supplier,GroupID,path,UserID):
        ...
        for row_index in range(7,table.nrows):
            for col_index in range(0,num_cols):
                ...
                PartNo = str(table.cell(row_index, 8).value)
                PartQuility = table.cell(row_index, 14).value
                PartTotalPrice = table.cell(row_index, 17).value

                strTurnDate = xlrd.xldate_as_tuple(table.cell_value(row_index, 25), data.datemode)
                _TurnDate = datetime(*strTurnDate[0:3]).strftime('%Y-%m-%d')
                TurnDate = datetime.strptime(_TurnDate, '%Y-%m-%d')

                strShipmentDate = xlrd.xldate_as_tuple(table.cell_value(row_index, 24), data.datemode)
                ...
                for x in result:
                    if aes.AESdecrypt('p@ssw0rd', x[1], True) == Name.encode('utf-8'):
                        SaleSQL = (
                            ...
                            x[1], PartQuility, PartTotalPrice, None, ShipmentDate, TurnDate, ShipmentDate,
            ...
            if (Ordernum==OrderNo[0:13]):
                ...
                self.updataOrder(mongoOrder,OrderNo,PartName,PartNo,\
                                PartQuility,PartTotalPrice,ShipmentDate,TurnDate,\
                                firm,supplier)
    ...
    def updataOrder(self,mongoOrder,_OrderNo,_PartName,_PartNo,\
        ...
        mongoOrder.cursor.update({ "OrderNo" : _OrderNo,'firm':_firm,'supplier':_supplier},\
                           ...
                           'PartQuility':_PartQuility,'Price':_PartTotalPrice,'ShipmentDate':_ShipmentDate,'TurnDate':_TurnDate}}
                                                )
